...
public class Session {
    ...
    private final List<OnContextsChangedListener> contextsChangedListeners = new LinkedList<>();
    ...
    //helper
    private void notifyContextAdded(Context context){
        for(OnContextsChangedListener listener : contextsChangedListeners){
            listener.contextAdded(context);
        }
    }

    private void notifyContextDeleted(Context context){
        for(OnContextChangedListener listener : contextChangedListeners){
            listener.contextDeleted(context);
        }
    }

    private void notifyContextChanged(Context context){
        for(OnContextsChangedListener : contextsChangedListeners){
            listener.contextChnaged(context);
        }
    }

    private void notifyContextsChanged(){
        for(OnContextsChangedListener listener : contextsChangedListeners){
            listener.contextChnaged();
        }
    }
    ...
    public interface OnContextsChangedListener {
        ...
        //called whenever the contexts is changed 
        default void contextChanged(Context context){}
